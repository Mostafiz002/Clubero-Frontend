...
const AdminOverview = () => {
  ...
  const grouped = (data.payments || []).reduce((acc, payment) => {
    const date = format(new Date(payment.paidAt), "dd MMM");
    ...
    <div className="p-6 space-y-8 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statsCards.map((card, i) => (
          ...
          >
            <div className={`${card.bg} ${card.color} p-4 rounded-xl text-2xl`}>
              <card.icon />
            </div>
            <div>
            ...
            <AreaChart data={chartData}>
              <defs>
                <linearGradient id="colorAmt" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#ff4a79" stopOpacity={0.1} />
                  <stop offset="95%" stopColor="#ff4a79" stopOpacity={0} />
                </linearGradient>
              </defs>
              <CartesianGrid
                strokeDasharray="3 3"
