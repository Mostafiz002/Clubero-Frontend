...
const ManageUsers = () => {
  ...
  const {
    ...
    queryKey: ["admin-dashboard-users", user?.email, searchText],
    enabled: !!user?.email,
    inputValue,
    queryFn: async () => {
  ...
  const handleRoleChange = async (id, role) => {
    try {
      await axiosSecure.patch(`/admin/role/${id}?role=${role}`);
  ...
  const handleManagerApplication = (id, becomeCM) => {
    axiosSecure.patch(`/admin/manageCM/${id}?becomeCM=${becomeCM}`).then(() => {
              ...
              <tbody>
                {applications.map((app) => (
                  ...
                  >
                    <td>
                      <div className="flex items-center gap-3">
                        <div className="avatar">
                          <div className="w-9 rounded-full ring ring-amber-200 ring-offset-2">
                            <img src={app.photoURL} alt={app.displayName} />
                          </div>
                        </div>
                    ...
                    <td className="text-slate-500 italic">{app.email}</td>
                    <td>
                      <span className="badge badge-warning badge-outline font-bold text-[10px] uppercase">
                        Club Manager
                      </span>
                    </td>
                    <td className="flex justify-center gap-3">
                      <button
                        onClick={() => {
                          handleRoleChange(app._id, "club-manager");
                          handleManagerApplication(app._id, "approved");
                        }}
                        className="btn btn-circle btn-sm btn-success text-white shadow-lg shadow-green-100"
                        title="Approve"
                      >
                      ...
                      <button
                        onClick={() =>
                          handleManagerApplication(app._id, "rejected")
                        }
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center  mt-6">
          <h2 className="heading relative">
            ...
            <span className="inline-block absolute -top-3 left-50">
              <svg
          ...
          <form
            ...
            className="input rounded-full bg-transparent outline-none hover:border-black/40 focus:border-black/40 flex items-center gap-2"
          >
            <svg
            ...
            <button
              type="submit"
              className="bg-gray-500/5 p-2.5 -mr-2 rounded-full"
            >
              <CiSearch />
            </button>
          </form>
                  ...
                  <td className="text-center align-middle">
                    <div className="flex gap-2 justify-center items-center">
                      ...
                      {u.role !== "club-manager" && (
                        <button
                          onClick={() =>
                            handleRoleChange(u._id, "club-manager")
                          }
